...
def _build_notify_url() -> Optional[str]:
    ...
    if _is_public_base_url(backend_base_url):
        return f"{backend_base_url}/api/wechat/notify"
    return None


def _is_mock_provider() -> bool:
    return _get_payment_provider().provider_name == "mock_wechat"
...
async def create_order(
    ...
    try:
        ...
        if provider.provider_name == "wechat" and not notify_url:
            logger.warning("WECHAT_NOTIFY_URL / BACKEND_BASE_URL 未配置为公网地址，微信支付异步通知可能无法到达")
