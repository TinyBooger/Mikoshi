...
// with what the highlighter and CodeBlock really support.
const FENCE_LANGUAGES = [...new Set([
  ...
  // Not a highlight.js grammar — CodeBlock routes this fence to MermaidDiagram.
  'mermaid',
])].sort().join(', ');
 ...
 * HTML, math instead of paired dollar signs) rather than generic Markdown lore.
 */
...
- 图表：语言名写 mermaid 的代码块会渲染为流程图、时序图、类图、状态图、甘特图等；语法有误时自动回退为普通代码块。
