...
function SearchPage() {
  ...
  const getSearchSortFromSubTab = (tab, subTab) => {
    if (tab === 'users') {
      return subTab === 'recent_updated' ? 'recent_updated' : 'total_rank';
    }
    if (subTab === 'popular') return 'popularity';
    if (subTab === 'recent') return 'recent';
    return 'relevance';
  };

  const getSubTabFromSearchSort = (tab, sort) => {
    if (tab === 'users') {
      if (sort === 'recent' || sort === 'recent_updated') return 'recent_updated';
      return 'total_rank';
    }
    if (sort === 'popularity' || sort === 'popular') return 'popular';
  ...
  // Initialize URL-driven search state
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setActiveSubTab(getSubTabFromSearchSort(activeMainTab, sort));
  }, [location.search, activeMainTab]);
  ...
  const handleMainTab = (tab) => {
    ...
    const params = new URLSearchParams(location.search);
    params.delete('page');
    params.set('sort', getSearchSortFromSubTab(tab, getSubTabFromSearchSort(tab, params.get('sort') || '')));
  ...
  const handleSubTab = (sub) => {
    ...
    const params = new URLSearchParams(location.search);
    params.delete('page');
    params.set('sort', getSearchSortFromSubTab(activeMainTab, sub));
    navigate({ pathname: '/search', search: params.toString() });
  };
  ...
  useEffect(() => {
    ...
    const sort = getSearchSortFromSubTab(activeMainTab, activeSubTab);

    let url = '';
    if (activeMainTab === 'characters') {
      url = `${window.API_BASE_URL}/api/characters/search`;
    } else if (activeMainTab === 'scenes') {
      url = `${window.API_BASE_URL}/api/scenes/search`;
    } else if (activeMainTab === 'personas') {
      url = `${window.API_BASE_URL}/api/personas/search`;
    } else if (activeMainTab === 'users') {
      url = `${window.API_BASE_URL}/api/search/users`;
    }
    const params = new URLSearchParams({ q, sort, short: 'false', page: String(page) });
  ...
  }, [activeMainTab, activeSubTab, sessionToken, navigate, page, location.search]);
