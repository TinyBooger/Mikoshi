...
export default function PersonaCreatePage() {
  ...
  // Removed traits, only tags are used
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!idToken) {
      alert('You need to be logged in to create a persona.');
      navigate('/');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.message || data.detail || 'Failed to create persona');
      }
    } catch (error) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 className="fw-bold text-dark mb-4" style={{ fontSize: '2.1rem', letterSpacing: '0.5px' }}>Create New Persona</h2>
        <form onSubmit={handleSubmit} className="w-100" encType="multipart/form-data">
          ...
          {/* Traits removed, only tags are used */}
          ...
          >
            <i className="bi bi-save me-2"></i>Save Persona
          </button>
        </form>
      </div>
    </div>
