                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(auto-fill, minmax(180px, 1fr))', gap: isMobile ? '0.5rem' : '1rem', justifyItems: 'stretch', padding: isMobile ? '0 0.25rem' : 0 }}>
                  {searchResults.length > 0 ? (
                    searchResults.map(persona => (
                      <EntityCard key={persona.id} type="persona" entity={persona} onClick={() => onSelect(persona)} />
                    ))
                  ) : (
                    <div className="text-muted text-center" style={{ gridColumn: '1 / -1' }}>{t('persona_modal.no_personas_found')}</div>
                  )}
                </div>
              )
            ) : (
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(auto-fill, minmax(180px, 1fr))', gap: isMobile ? '0.5rem' : '1rem', justifyItems: 'stretch', padding: isMobile ? '0 0.25rem' : 0 }}>
                  {popularPersonas.length > 0 ? (
                    ...
                    <div className="text-muted text-center" style={{ gridColumn: '1 / -1' }}>{t('persona_modal.no_popular_personas')}</div>
                  )}
                </div>
              </>
            )}
